...
window.addEventListener('load', function () {
    ...
    if (maxUnlocked < 20) {
        showToast("Level 20 is the only Multiplayer level! You must clear Level 19 in Single Player to unlock Multiplayer.", "error", 5000);
        setTimeout(() => {
    ...
    class Game {
        ...
        _init() {
            ...
            const cfg = LEVEL_CONFIG[this.level] || LEVEL_CONFIG[20];
        ...
        _advanceWave() {
            ...
            this.enemyInterval = (LEVEL_CONFIG[this.level] || LEVEL_CONFIG[20]).enemyInterval;
            const wave = this.currentWave;
            if (wave) {
                const waveNames = {
    ...
    function setupMultiplayerAndAuth(game) {
        ...
        function connectSocket(authToken) {
            ...
            game.socket.on('connect_error', (err) => {
                console.error('Socket connection error:', err);
                if (err.data && err.data.code === 'LEVEL_NOT_UNLOCKED') {
                    showToast(err.message || 'You must complete Story Mode before joining Multiplayer.', 'error');
                    setTimeout(() => {
                        window.location.href = 'index.html';
                    }, 2500);
                } else {
    ...
    if (resetProgressBtn) {
        resetProgressBtn.addEventListener('click', () => {
            ...
            // Redirect back to index.html since progress is wiped
            showToast("Progress reset successfully! You must clear Level 19 in Single Player to access Multiplayer again.", "success", 5000);
            setTimeout(() => {
                window.location.href = 'index.html';
            }, 3000);
            return;
